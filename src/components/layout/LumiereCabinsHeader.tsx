...
import { UtensilsCrossed, ChevronDown, Menu, X } from "lucide-react";
...
export function LumiereCabinsHeader() {
  ...
  return (
    <header className="w-full bg-[#0a0a0a] border-b border-white/10">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">

        {/* Logo */}
        <Link href="/" className="flex items-center gap-2.5 text-white">
          <UtensilsCrossed className="h-5 w-5" strokeWidth={1.5} />
          <span className="font-semibold text-base tracking-tight">LumièreCabins</span>
        </Link>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="flex items-center gap-2 text-white/80 hover:text-white transition-colors outline-none">
                <Avatar className="h-7 w-7">
                  ...
                  <AvatarFallback className="text-xs bg-white/10 text-white border border-white/10">
              ...
              <DropdownMenuContent align="end" className="w-48 bg-[#1a1a1a] border-white/10 text-white">
                ...
                <DropdownMenuItem asChild className="focus:bg-white/10 focus:text-white text-white/70 cursor-pointer">
              ...
              {session?.user ? (
                <>
                  ...
                  <button
                    ...
                    className="flex-1 rounded-md px-3 py-2 text-sm text-center text-red-400 border border-white/10"
                  >
                    Sign out
                  </button>
                </>
                ...
                <>
                  ...
                  <Link
                    href="/register"
                    onClick={() => setMobileOpen(false)}
                    className="flex-1 rounded-md px-3 py-2 text-sm text-center text-white border border-white/30 bg-white/5"
                  >
                    Sign Up
                  </Link>
                </>
